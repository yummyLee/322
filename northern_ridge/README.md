...
新地形范围 X=-112…-26，Z=-82…-42；可行走边界延伸到 Z=-77，西界延续 X=-87。地形在 Z=-42 与旧地形对齐，过渡区向北逐渐增加缓坡和红土。顶点 RGBA 保存草地、矿土、泥土、腐叶土权重；地面使用多尺度色差、柔边苔斑、成簇杂草、叶屑与半埋石，避免纯色地板。

旧墓地北界碰撞已开放；东侧连接处移动六棵旧树及对应树干碰撞。道路沿实际地面起伏铺设，有不规则路肩、磨损、浅车辙、碎石和断续铺石。门洞按 1.86 单位高的实际玩家校准，约 1.64 单位净宽、2.31 单位门楣净高。
...
2026-09-18，Godot 4.7.2、D3D12 Forward+、RTX 5080。

- `tools/validate.gd`：原入口集成、实际节点归属、183 个新旧边缘顶点的高度/权重连续、55 处地形碰撞采样、四条实际玩家通行测试（含石门往返）、编辑后的磁盘保存重载。
- `tools/verify_gpu.gd`：通过原入口加载场景，检查原出生点、分阶光照开关、整数像素缩放、菜单暂停与恢复；输出并人工查看像素、普通 3D、近景和缓坡截图。
