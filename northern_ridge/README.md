...
| EastNorthExtension | 洞口以东、村北 | 按《地图草稿甲》重排的东向连续地形、泉眼水沟、果树林、村北猎户营地、北路绳界与东侧林缘 |
| WindingTrails | 多组道路 | 墓地至路亭、石门支路、洞口至泉眼、北侧林道、村北斜坡、猎户营地与果林小径 |
...
旧墓地北界碰撞已开放；东侧连接处移动六棵旧树及对应树干碰撞。道路沿实际地面起伏铺设，有不规则路肩、磨损、浅车辙、碎石和断续铺石。根缠石门采用北高南低的连续地形，洞口两侧为缓坡折面，根系逐点贴合地形高度。东向新增地形从 X=-26 的共享边界继续到 X=26，主路按参考图连接泉眼、北侧林道、猎户营地、果林和窑村北缘。东扩南缘在 Z=-42…-32 做连续高差过渡，逐步下接窑村原地面；过渡面使用同一套地形网格、材质权重和碰撞采样，避免新区像单独拼上的平板。村北营地使用兽皮晒架、悬挂皮张、火塘、剥皮台和简易猎户帐篷，替换原先不符合设定的洗衣架。门洞按 1.86 单位高的实际玩家校准，约 1.64 单位净宽；洞内地板至顶面净高约 2.23 单位。

## 预留洞室入口

`RootWrappedStoneGate/ReservedEntrance` 仍保留为可编辑的预留洞口，但当前已停用。`burial_left_cave` 已删除，后续重新制作洞穴时可从此入口恢复传送配置。
...
- `tools/validate.gd`：原入口集成、实际节点归属、185 个新旧边缘顶点的高度/权重连续、104 处地形碰撞采样、旧墓地至石门和东扩各路线的实际玩家通行测试、窑村北缘斜坡跨越、洞顶覆土和通道身体/头部净空、编辑后的磁盘保存重载。
...
日常直接修改保存的场景；移动地形和主要物体时同步调整碰撞。修改共享材质或网格前按需要将资源唯一化。`tools/extend_north.gd`、`tools/extend_east_saved.gd` 和 `tools/repair_east_saved.gd` 是离线制作留档，不作为运行依赖；东扩修复脚本带版本保护，重新烘焙会覆盖新区手动修改。`integrate_north.gd` 和 `link_saved_scene.py` 已应用，有重复执行保护。不要重跑原 `village/tools/build_village.gd`。
