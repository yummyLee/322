...
布局按参考图和 J0～J13 场景描述重建为直接世界坐标：南侧 J0 入口，中央 J8 旧车场，左侧 J3/J9 湿路与石柱林，西北 J6 竖井，北侧 J5 码骨厅和 J10 灰瀑阶地，东侧 J11 悬骨桥、J12 采石廊和 J13 记名壁室。所有可行走区块和主通路统一在 `Y=0` 基准面；洞壁向下挤出形成厚度，只有瀑布阶、深井、桥栏和钟乳石使用额外高度。每个区块都是独立的不规则地面、多层挤出崖壁和底部碎石，区块之间只用连续窄通路连接，黑色裂谷保持为空。新版加入了连续洞壁带、区块分隔墙、石柱拱口、多环变半径石柱、四级瀑布、断桥深井和按区块摆放的骨架/矿车/木架/墓碑物件。完整的区块拆分与路线设计见 `ZONE_DESIGN.md`。地图外缘有不可见边界墙，底部有安全接住面，避免角色掉出场景。
